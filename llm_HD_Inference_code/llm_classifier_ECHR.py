...
HALL_CLF_PATH = "saved_model/saved_model_gemma_ECHR/att_last3.pth"
...
results_dir = Path("./results_echr_test_HD1/")
...
def load_data_from_csv(csv_path):
        ...
        # sample_df = df.sample(n=min(1000, len(df)), random_state=42, replace=False)

        # end = 850 #end or len(df)
        ...
        for i in range(len(df)):
            ...
            user_content = (
                "Based on the European Convention on Human Rights, determine whether "
                "the following case proceeding shows a violation of a human rights article.\n\n"
                f"Case Proceeding:\n{ex['text']}\n\n"
                "Decision:"
            )
            ...
            answer = label_map[ex["binary_judgement"]]
...
def compute_and_save_results():
    ...
    dataset = load_data_from_csv("Datasets/test_ECHR_Dataset_binary.csv")
    ...
    # -------------------- SAVE PATH --------------------
    out_dir = "results_llm_HD/ECHR"
    os.makedirs(out_dir, exist_ok=True)
    first_word = args.model_name.split("-")[0]
    out_file = os.path.join(
        out_dir,
        f"gated_results_{first_word}_{datetime.now().strftime('%m%d_%H%M')}.json"
    )
